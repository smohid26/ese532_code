#ifndef SRC_CONSTANTS_
#define SRC_CONSTANTS_

#define INPUT_HEIGHT_SCALE (540)
#define INPUT_WIDTH_SCALE (960)
#define FRAME_SIZE (INPUT_WIDTH_SCALE * INPUT_HEIGHT_SCALE)
#define FRAMES (200)
#define MAX_OUTPUT_SIZE (5000 * 1024 * 10)
#define FILTER_INPUT_HEIGHT (270)
#define FILTER_INPUT_WIDTH (480)

#define FILTER_LENGTH (7)

#define FILTER_OUTPUT_HEIGHT (FILTER_INPUT_HEIGHT - (FILTER_LENGTH - 1))
#define FILTER_OUTPUT_WIDTH (FILTER_INPUT_WIDTH - (FILTER_LENGTH - 1))
#endif